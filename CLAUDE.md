...
**CSS scroll-snap does not work here either**, and for exactly the same reason — the browser has no real scroll offset to snap to. Section snapping is done with `ScrollTrigger`'s `snap` option, which drives `ScrollTrigger.scroll()` itself. Full-page/one-section-per-viewport scrolling is also off the table: both scroll-driven sections need multiple viewports of scroll distance to play their sequences.
...
- `components/DefinitionSection.tsx` — a `300vh` pinned section where a circular image mask (the "ikra." wordmark's dot) scales up to a computed `maxScale` — the farthest viewport-corner distance from the circle's center, with a 15% margin — until it covers the full viewport and releases into the footer. It also owns a **second, snap-only `handoff` ScrollTrigger** covering the 100vh between the hero's pin releasing and this section reaching the top, so the transition settles instead of drifting halfway. That range is deliberately the gap where *neither* section's pin is active: putting `snap` on either scrubbed trigger would yank its animation to one end whenever the user paused mid-reveal.
